...
export default class RecordingView extends View {
    ...
    set speechDetected(value) {
        if (this.view)
            this.view.speechDetected = value;
    }
    /**
     * @param {boolean} value
     */
    ...
    constructor(controller, recorder) {
        super(controller);
        if (!recorder)
            throw new Error("Recorder not given to RecordingView");
    ...
    updateView() {
        ...
        if (Constructor) {
            if (this.view) {
                ...
                this.view = new Constructor(this.controller, this.formantsContainer);
        }
    }
    feed(samples) {
