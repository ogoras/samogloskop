...
export default class StatsView extends SpeechView {
    ...
    set intensityStats(statsGiven) {
        ...
        if (stats.min.diff !== undefined)
            stats.min.diff = statsGiven.diff(0);
        stats.max.value = statsGiven.max;
        if (stats.max.diff !== undefined)
            stats.max.diff = statsGiven.diff(1);
        stats.mean.value = statsGiven.mean;
        if (stats.mean.diff !== undefined)
            stats.mean.diff = statsGiven.diff(2);
        if (stats.range !== undefined)
            stats.range.value = statsGiven.range;
