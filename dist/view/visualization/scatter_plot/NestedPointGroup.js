    ...
    navigate(ids, createMode = CREATE_MODES.DONT, constructorDefaults) {
        ...
        switch (createMode) {
            ...
            case CREATE_MODES.INSERT:
                if (ids.length == 1) {
                    subgroup = this.insertSubgroup(ids[0], constructorDefaults, constructorDefaults?.nested);
                    break;
                }
            // FALLTHROUGH otherwise
            case CREATE_MODES.IF_NOT_EXISTS:
                subgroup = this.getOrCreateSubgroup(ids[0], constructorDefaults, constructorDefaults?.nested || ids.length > 1);
                break;
        }
        if (!subgroup)
            throw new Error(`navigate: subgroup ${ids[0]} od ${this.id} not found`);
        else if (subgroup.constructor === SimplePointGroup) {
            if (ids.length > 1)
                throw new Error(`navigate: subgroup ${subgroup.id} is a SimplePointGroup and \
cannot have subgroups`);
            ...
            if (ids.length == 1)
                ...
                return subgroup.navigate(ids.slice(1), createMode, constructorDefaults);
        }
        else {
