...
export default function resample(samples, oldSampleRate, newSampleRate, precision = 50) {
    ...
    if (Math.abs(upfactor - 2) < 1e-6)
        return upsample(samples);
    if (Math.abs(upfactor - 1) < 1e-6)
        return samples;
    try {
        ...
        if (numberOfSamples < 1)
            throw new Error("The resampled sound would have no samples.");
        ...
        if (weNeedAnAntiAliasingFilter) {
            ...
            for (let i = Math.floor(upfactor * nfft) - 2; i < nfft; i++)
...
function upsample(samples) {
    try {
        ...
        */
        ...
        for (let i = 0; i < numberOfSamples; i++)
            newSamples[i] = data[i + sampleRateFactor * antiTurnAround] * factor;
        return newSamples;
    }
    catch (error) {
