...
export function Polynomial_to_Roots(polynomial) {
    try {
        if (polynomial.numberOfCoefficients != polynomial.coefficients.length)
            throw new Error("Number of coefficients mismatch!"); // check invariant
        let np1 = polynomial.numberOfCoefficients, n = np1 - 1;
        if (n <= 0)
            throw new Error("Cannot find roots of a constant function.");
        /*
        ...
        */
        ...
        let pn = emlapack._malloc(4), p1 = emlapack._malloc(4), ph = emlapack._malloc(n * n * 8), pwr = emlapack._malloc(n * 8), pwi = emlapack._malloc(n * 8), pwtmp = emlapack._malloc(8), plwork = emlapack._malloc(4), pinfo = emlapack._malloc(4);
        ...
        */
        ...
        if (info > 0) {
            ...
            */
            ...
            if (numberOfEigenvaluesFound <= 0)
                throw new Error("No eigenvalues found.");
            ioffset = info;
        }
        else if (info < 0) {
