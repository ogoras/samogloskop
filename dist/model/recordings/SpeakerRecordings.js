...
export default class SpeakerRecordings extends ArrayLoadedFromFile {
    ...
    async _load() {
        const dir = `./data/recordings/${this.language}/${this.speaker}`;
        ...
        const recordings = await Promise.all(recording_names.map(async (recording_name) => await Recording.create(`${dir}/${recording_name}`, info)));
        this.push(...recordings);
        this.vowels.gatherMeasurements(this.flatMap(recording => recording.getVowelMeasurements(this.vowels.getVowelSymbols())));
    }
}
