...
import { vowelLetterToIndex } from '../../const/VOWEL_INVENTORIES.js';
...
export default class SpeakerVowels extends Vowels {
    ...
    get meanFormants() {
        ...
        if (!this.#meanFormants)
            throw new Error("Mean formants are not defined");
    ...
    gatherMeasurements(measurements) {
        measurements.forEach(measurements => {
            ...
            if (!vowel)
                throw new Error(`Could not find vowel ${measurements.vowel} in SpeakerVowels`);
    ...
    get formantsDeviation() {
        ...
        if (!this.#formantsDeviation)
            throw new Error(`Formants deviation is undefined in ${this.constructor.name}`);
    ...
    calculateMeanFormantsDeviation() {
        const varianceTimesN = this.vowelsProcessed.reduce((acc, vowel) => {
            return {
    ...
    addFormants(formants) {
        if (!this.currentVowel)
            throw new Error("No current vowel");
        if (!formants)
    ...
    saveVowel() {
        if (!this.currentVowel)
            throw new Error(`Could not save vowel: No current vowel`);
    ...
    scaleLobanov() {
        if (this.lobanovScaled && this.#scaleCurrent)
            return;
    ...
    static fromString(string, language = "PL", scaleLobanov = true) {
        ...
        speakerVowels.vowels = speakerVowels.vowelsProcessed = obj.vowelsProcessed.map((vowel) => Vowel.fromSimpleObject({ ...vowel, language }));
        if (!obj.lobanovScaled) {
            if (scaleLobanov)
                speakerVowels.scaleLobanov();
        }
        else {
