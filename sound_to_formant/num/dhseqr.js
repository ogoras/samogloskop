...
*/
export function dhseqr_(n, ihi, h__, ldh, ldz, work, lwork)
{
    const job = "E";
    const compz = "N";
    const ilo = 1;
    ...
    char *a__1[2];
    let h_dim1, h_offset, z_dim1, z_offset, i__1, i__2[2], i__3;
    let d__1;
    char ch__1[3]; 

    /* Local variables */
    let i__;
    let hl[2401]	/* was [49][49] */;
    let kbot, nmin;
    let initz;
    let workl[49];
    let wantt, wantz;
...
/*     H     (input/output) let PRECISION array, dimension (LDH,N) */
...
/*     WR    (output) let PRECISION array, dimension (N) */
/*     WI    (output) let PRECISION array, dimension (N) */
...
/*     Z     (input/output) let PRECISION array, dimension (LDZ,N) */
...
/*     WORK  (workspace/output) let PRECISION array, dimension (LWORK) */
...
/*                       are passed to the implicit let shift routine */
...
/*     ==== Decode and check the input parameters. ==== */
    ...
    if (job != "E" && ! wantt) {
	    ...
	    info = -7;
    } else if (ldh < 1 || wantz && ldh <Math.max(1,n)) {
	    info = -11;
...
/*        ==== Quick return in case of a workspace query ==== */
        ...
        dlaqr0_(wantt, wantz, n, ilo, ihi, h__[h_offset], ldh, wr, wi, ilo, ihi, z__[z_offset], ldz, work, lwork, info);
